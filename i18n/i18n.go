...
func LangCodePrefer(uid int64, defaultLang string) string {
	userLang, err := DB.Init("lang", uid, nil)
	if err != nil {
		return defaultLang
	}
	langCode, ok := userLang["display_language"].(string)
	if !ok || langCode == "" {
		return defaultLang
	}
	return langCode
...
func QueryLangCodePrefer(uid int64, defaultLang string) string {
	userLang, err := DB.Init("lang", uid, nil)
	if err != nil {
		return defaultLang
	}
	langCode, ok := userLang["query_language"].(string)
	if !ok || langCode == "" {
		return defaultLang
	}
	return langCode
